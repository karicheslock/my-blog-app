...
function Home({isAuth, title, postText, image}) {
                    ...
                    return (
                        <div key={post.id} className='flex flex-col mx-auto w-full py-5 px-4 items-center'>
                            ...
                            <div className='flex w-1/2 py-2'>
                                ...
                                <div>
                                    {isAuth && post.author.id === auth.currentUser.uid && <button className='text-white text-md flex ml-40 w-1/8 border-2 border-light-teal rounded bg-light-teal px-4 py-2' onClick={() => {deletePost(post.id)}}>Delete Above Post</button>}
                                </div>
                            </div>
                        </div>
                    )
